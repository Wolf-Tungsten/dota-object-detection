...
def multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    boxes_ids = (list(range(num_classes)) if len(per_class_boxes_list) > 1
                 else [0] * num_classes.value)
    for class_idx, boxes_idx in zip(list(range(num_classes)), boxes_ids):
      ...
      if additional_fields is not None:
        for key, tensor in list(additional_fields.items()):
  ...
  with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
    ...
    def _single_image_nms_fn(args):
      ...
      if per_image_additional_fields is not None:
        for key, tensor in list(per_image_additional_fields.items()):
